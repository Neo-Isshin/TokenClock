...
/// (cachedContentTokenCount)，故 `total = input + output + thought`，**不含 cached**
///（否则双计，同 Codex）。参见 `docs/TOOL_SCHEMA_ANALYSIS.md`。
///
...
final class GeminiUsageServiceTests: XCTestCase {
    ...
    func testTokenFormula_excludesCached_includesThought() {
        ...
        // input=1000（已含 cached=800）+ output=100 + thought=50 = 1150（非 1950）
        ...
        XCTAssertEqual(svc.dailyData[key]?.tokens, 1150,
                       "Gemini total = input+output+thought；cached 已含在 input 内，不可再加")
        XCTAssertEqual(svc.dailyCache[key], 800, "cached 仍计入 cache 统计（缓存率分母用新 total）")
    }

    func testTokenFormula_missingThought_defaultsZero() {
        let ts = "2026-07-10T12:00:00.000Z"
        // 无 thought 字段 → thought=0；total = 1000+100+0 = 1100（非 1900）
        writeSession(project: "p1", session: "session-s1.jsonl", lines: [
        ...
        svc.fullScan()
        let key = DateHelper.localDateKey(from: ts)
        XCTAssertEqual(svc.dailyData[key]?.tokens, 1100, "缺 thought 时按 0 计，仍不加 cached")
    }
