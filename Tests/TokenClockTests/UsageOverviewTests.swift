...
final class UsageOverviewTests: XCTestCase {
    ...
    func testProviderHistoryRepairPreservesOtherToolsAndRemovesStaleProviderDays() throws {
        ...
        store.upsertDay(dateKey: "2026-08-05", snapshots: [
            ToolSnapshot(name: "OpenClaw", tokens: 901_873, messages: 39, cacheRate: 1.08,
                         isActive: false),
            ToolSnapshot(name: "Codex", tokens: 123, messages: 2, cacheRate: 0,
                         isActive: false),
        ])
        store.upsertDay(dateKey: "2026-08-06", snapshots: [
            ToolSnapshot(name: "OpenClaw", tokens: 456, messages: 3, cacheRate: 0.5,
                         isActive: false),
            ToolSnapshot(name: "Codex", tokens: 789, messages: 4, cacheRate: 0,
                         isActive: false),
        ])
        let repaired = ToolSnapshot(
            name: "OpenClaw", tokens: 10, messages: 1, cacheRate: 0.25,
            isActive: false, cacheReadTokens: 3
        )
        store.replaceToolHistory(
            toolName: "OpenClaw", snapshotsByDate: ["2026-08-06": repaired],
        ...
        XCTAssertEqual(days.first(where: { $0.date == "2026-08-06" })?.tools.first(where: { $0.name == "Codex" })?.tokens, 789)
    }
