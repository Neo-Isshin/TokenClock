...
try {
    ...
    for($second=1;$second-le$SoakSeconds;$second++){
        ...
        $sample=[pscustomobject][ordered]@{phase='sustained-soak';at=$now.ToString('o');coreCpuPercent=[Math]::Round($core,3);systemCpuPercent=[Math]::Round($core/$logical,4);workingSetMiB=[Math]::Round($process.WorkingSet64/1MB,3);privateMiB=[Math]::Round($process.PrivateMemorySize64/1MB,3);threads=$process.Threads.Count;handles=$process.HandleCount;gdiObjects=[TCPerfNative]::GetGuiResources($process.Handle,0);userObjects=[TCPerfNative]::GetGuiResources($process.Handle,1);processGpuPercent=$gpu;responding=$process.Responding;hung=[TCPerfNative]::IsHungAppWindow($main)}
