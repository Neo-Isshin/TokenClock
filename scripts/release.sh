#!/usr/bin/env bash
#
# release.sh —— TokenClock 一键发版流水线（GitHub Releases）
#
# 把手工发版的整条链脚本化：
#   两分支 checkout → universal 构建 → ad-hoc 签名 → tar+SHA → 改 install.sh/tokenclock pin
#   → commit 两分支 → push → tag → gh release create → 上传资产 → 下载校验 SHA。
#
# 设计要点（踩过的坑全固化进来）：
#   * 发版鉴权走 gh（gh 自己管 GitHub 凭证，无需在脚本里解析/暴露 token）。
...
#   * --only 仅用于给已存在的 release 补传某一变体；全新版本号必须双变体都发（否则 installer 404）。
...
#   scripts/release.sh v1.3.0 --only normal # 只发 normal（glass 沿用旧二进制）—— 仅补传已存在的 release
...
# GitHub 仓库（公开主仓）；origin 也应指向它。gh 命令默认从 origin 推断 owner/repo，
# 此处显式声明仅用于拼校验下载直链。
GH_OWNER="Neo-Isshin"
GH_REPO="TokenClock"
GH_REPO_SLUG="${GH_OWNER}/${GH_REPO}"

# 构建配方（与 memory 记录一致）：
#   normal(.v12) 触发 Swift 回退库 → CLT 缺 x86_64 切片 → 必须 Xcode 工具链 + CLT 26 SDK
#   glass(.v26)  不触发回退库 + 26 SDK 里 @State 还是属性包装器 → CLT 26 SDK 直编
# XCODE_BETA 默认取 xcode-select -p（须是带 universal 回退库的完整 Xcode，非 CLT）；可环境变量覆盖。
XCODE_BETA="${XCODE_BETA:-$(xcode-select -p)}"
CLT26_SDK="${CLT26_SDK:-/Library/Developer/CommandLineTools/SDKs/MacOSX26.sdk}"
...
say()  { printf '%s\n' "$*"; }
# 注意：step/info/ok/warn 走 stderr —— build_variant() 用 $() 捕获返回值（末行 echo "$bin"），
# 若进度打到 stdout 会被一起吞进 BIN_ 变量（曾导致 BIN_GLASS 变成多行垃圾 → tar "File name too long"）。
# say() 是最终结果，留 stdout。
step() { printf '\n%s━━ %s ━━%s\n' "$C_CYAN" "$*" "$C_RESET" >&2; }
info() { printf '  %s%s%s\n' "$C_DIM" "$*" "$C_RESET" >&2; }
ok()   { printf '  %s✓ %s%s\n' "$C_GREEN" "$*" "$C_RESET" >&2; }
...
[ "$DO_NORMAL" = 1 ] && { (DEVELOPER_DIR="${XCODE_BETA}" xcrun --find swift >/dev/null 2>&1) || die "找不到 Xcode 的 swift（normal 构建需要 Xcode universal 回退库，非 CLT；设 XCODE_BETA 指向完整 Xcode）：${XCODE_BETA}"; }
[ -d "$CLT26_SDK" ] || die "找不到 CLT 26 SDK: $CLT26_SDK（设 CLT26_SDK 环境变量）"

command -v gh >/dev/null 2>&1 || die "找不到 gh（发版需要 GitHub CLI：brew install gh && gh auth login）"
gh auth status >/dev/null 2>&1 || die "gh 未登录，先 gh auth login"
ok "gh 已登录"

# --only 仅用于给【已存在】的 release 补传某一变体。全新版本号用 --only 会让被跳过的
# 变体在新 release 里缺席 tarball → installer 下载该变体时 404。提前挡住，避免静默产出残缺 release。
if [ -n "$ONLY" ]; then
  if gh release view "$VERSION" >/dev/null 2>&1; then
    ok "--only=$ONLY：目标 release 已存在，仅补传 $ONLY 变体（另一变体沿用既有资产）"
  else
    die "--only 不能用于全新版本号：$VERSION 的 release 当前不存在。全新版本号必须双变体都发（去掉 --only），否则另一变体 tarball 缺席 → installer 404。--only 仅适合给已存在的 release 补传某一变体。"
  fi
fi

# ─────────────────────────── 构建 ───────────────────────────
# build_variant <branch> <recipe-tag>  → 产物落到 .build/...（fat），stdout 输出二进制路径
build_variant() {
  ...
  case "$recipe" in
    ...
    normal)
      ...
      info "DEVELOPER_DIR=$XCODE_BETA SDKROOT=${CLT26_SDK}（.v12 触发回退库，需 Xcode universal 切片）" ;;
  esac
...
# tarball 用最终资产名，gh release upload 以 basename 作 asset 名。
TAR_NORMAL="/tmp/TokenClock-normal-universal.tar.gz"
TAR_GLASS="/tmp/TokenClock-glass-universal.tar.gz"
...
if [ "$DO_NORMAL" = 1 ]; then
  ...
  tar -C "$(dirname "$BIN_NORMAL")" -cf "${TAR_NORMAL%.gz}" $NORMAL_PKG || die "normal tar 失败: $BIN_NORMAL"
  gzip -n -f "${TAR_NORMAL%.gz}"
  SHA_NORMAL="$(shasum -a 256 "$TAR_NORMAL" | cut -d' ' -f1)"
  [ -n "$SHA_NORMAL" ] || die "normal SHA 为空（tarball 未生成: $BIN_NORMAL）"
  ok "normal tarball: $TAR_NORMAL  SHA=${SHA_NORMAL:0:16}…"
fi
if [ "$DO_GLASS" = 1 ]; then
  ...
  tar -C "$(dirname "$BIN_GLASS")" -cf "${TAR_GLASS%.gz}" $GLASS_PKG || die "glass tar 失败: $BIN_GLASS"
  gzip -n -f "${TAR_GLASS%.gz}"
  SHA_GLASS="$(shasum -a 256 "$TAR_GLASS" | cut -d' ' -f1)"
  [ -n "$SHA_GLASS" ] || die "glass SHA 为空（tarball 未生成: $BIN_GLASS）"
  ok "glass tarball:  $TAR_GLASS  SHA=${SHA_GLASS:0:16}…"
fi
...
if [ "$NORMAL_CHANGED" = 0 ] && [ "$GLASS_CHANGED" = 0 ] && [ "$TAGS_CHANGED" = 0 ]; then
  if gh release view "$VERSION" >/dev/null 2>&1; then
    ...
    warn "SHA 未变但 release 不存在 —— 继续走发布步骤创建 release"
  fi
fi
...
# ─────────────────────────── GitHub release（gh）───────────────────────────
step "GitHub release"
NOTES="TokenClock ${VERSION}"
if gh release view "$VERSION" >/dev/null 2>&1; then
  info "release 已存在，更新 notes"
  gh release edit "$VERSION" --notes "$NOTES" >/dev/null || die "gh release edit 失败"
else
  info "创建 release（target=${BR_GLASS}）"
  gh release create "$VERSION" --target "$BR_GLASS" --title "$VERSION" --notes "$NOTES" >/dev/null || die "gh release create 失败"
fi
ok "release $VERSION 就绪"

# ─────────────────────────── 上传资产（gh release upload --clobber 覆盖同名）───────────────────────────
step "上传资产"
UPLOAD=()
[ "$DO_NORMAL" = 1 ] && UPLOAD+=("$TAR_NORMAL") || info "normal 本次未构建（--only glass），沿用既有 release 资产"
[ "$DO_GLASS" = 1 ]  && UPLOAD+=("$TAR_GLASS")  || info "glass 本次未构建（--only normal），沿用既有 release 资产"
...
if [ "${#UPLOAD[@]}" -gt 0 ]; then
  gh release upload "$VERSION" "${UPLOAD[@]}" --clobber >/dev/null || die "gh release upload 失败"
  ok "已上传 ${#UPLOAD[@]} 个资产"
fi
...
step "下载校验"
verify() {
  local name="$1" want="$2" got url
  [ -n "$want" ] || return 0
  url="https://github.com/${GH_REPO_SLUG}/releases/download/${VERSION}/${name}"
  for i in 1 2 3 4 5 6; do
    if curl -fsSL --max-time 120 "$url" -o /tmp/tc-verify.tar.gz 2>/dev/null; then break; fi
    warn "$name 下载第 $i 次失败，退避…"; sleep $((i*2))
  done
  [ -s /tmp/tc-verify.tar.gz ] || die "$name 下载失败（6 次均失败）：$url"
...
say "  tag        : $VERSION"
say "  release    : https://github.com/${GH_REPO_SLUG}/releases/tag/${VERSION}"
...
say "  one-liner  : curl -fsSL https://raw.githubusercontent.com/${GH_REPO_SLUG}/${BR_GLASS}/cli/install.sh | bash"
