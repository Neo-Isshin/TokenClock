...
///
/// The scheduler is intentionally UI-free: each platform can surface the returned
/// notifications in its native shell while using the same date boundaries and routes.
enum UsageReportScheduler {
    ...
    private static func generatePendingWeeklyReports(through completedDate: Date) -> [TokenClockNotification] {
        ...
        var weekEnd: Date
        if let last = lastKey.flatMap(date(from:)), let next = calendar.date(byAdding: .day, value: 7, to: last) {
            weekEnd = next
        } else {
            let weekday = calendar.component(.weekday, from: completedDate) // Sunday = 1
    ...
    private static func generatePendingMonthlyReports(through completedDate: Date) -> [TokenClockNotification] {
        ...
        var monthEnd: Date
        if let last = lastKey.flatMap(date(from:)),
           let firstOfNext = calendar.date(byAdding: .day, value: 1, to: last),
           let firstOfFollowing = calendar.date(byAdding: .month, value: 1, to: firstOfNext),
           let nextEnd = calendar.date(byAdding: .day, value: -1, to: firstOfFollowing) {
            monthEnd = nextEnd
        } else {
            let nextDay = calendar.date(byAdding: .day, value: 1, to: completedDate) ?? completedDate
            if calendar.component(.month, from: nextDay) != calendar.component(.month, from: completedDate) {
        ...
        while monthEnd <= completedDate, count < 24 {
            ...
            guard let firstOfNext = calendar.date(byAdding: .day, value: 1, to: monthEnd),
                  let firstOfFollowing = calendar.date(byAdding: .month, value: 1, to: firstOfNext),
                  let nextEnd = calendar.date(byAdding: .day, value: -1, to: firstOfFollowing) else { break }
            monthEnd = nextEnd
            count += 1
        }
        return reports
    ...
    private static func report(
        ...
        return TokenClockNotification(
            kind: kind,
            title: L10n.shared.tr(titleKey),
            message: L10n.shared.tr(
                "notification.dailyReportMessage", label,
                TokenFormat.compact(tokens), messages
            ),
            route: route
        )
    }
