...
/// Credentials stay in memory only and are never logged or persisted by TokenClock.
final class ClaudeQuotaService: @unchecked Sendable {
    ...
    static func credentialCandidatePaths(
        ...
        return directories.compactMap {
            let path = (((($0 as NSString).expandingTildeInPath) as NSString)
    ...
    static func decodeUsageResponse(
        ...
        for definition in definitions {
            ...
            buckets.append(CodexQuotaBucket(
                id: "claude:\(definition.key)", name: definition.name, usedPercent: used,
                windowMinutes: definition.minutes, resetsAt: date(value["resets_at"])
            ))
        }
        guard !buckets.isEmpty else { return nil }
        let order = Dictionary(uniqueKeysWithValues: definitions.enumerated().map { ($0.element.key, $0.offset) })
        buckets.sort {
            let lhs = String($0.id.dropFirst("claude:".count)), rhs = String($1.id.dropFirst("claude:".count))
    ...
    private func readMacOSKeychainCredential(
        ...
        return Self.decodeCredentialPayload(output.fileHandleForReading.readDataToEndOfFile())
    }
    #endif
