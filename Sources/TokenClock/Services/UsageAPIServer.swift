    ...
    private func sendUsageJSON(_ connection: NWConnection) {
        Task { @MainActor in
                ...
                if !tool.sessions.isEmpty {
                    dict["sessions"] = tool.sessions.map { session -> [String: Any] in
                        [
                            ...
                            "todayCacheReadTokens": session.cacheReadTokens,
                            "todayMessages": session.todayMessages,
                            "todayCost": session.todayCost.value,
                            "costComplete": session.todayCost.complete,
                            "costAvailable": session.todayCost.available,
            ...
            let body: [String: Any] = [
                ...
                "displayTotalTokens": vm.displayTotalTokens,
