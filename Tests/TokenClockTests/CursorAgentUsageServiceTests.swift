...
final class CursorAgentUsageServiceTests: XCTestCase {
    ...
    func testGrokBotModelsAreSeparatedFromDirectCursorUsage() {
        ...
        service.applyEvents([
            event(timestamp: timestamp, model: "claude-fable-5-medium", input: 100, output: 20, cacheRead: 300),
        ...
        XCTAssertEqual(service.todaySessions().map(\.displayName).sorted(), ["claude-fable-5", "grok-4"])
    ...
    func testCursorVendorPrefixesCollapseToUnderlyingModelFamilies() {
