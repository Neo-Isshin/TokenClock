    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            ScrollView([.horizontal, .vertical], showsIndicators: false) {
                let rows = Array(stride(from: 0, to: ClockFaceTheme.allCases.count, by: 3))
                VStack(spacing: 12) {
                    ForEach(rows, id: \.self) { rowStart in
                        HStack(spacing: 20) {
                            ForEach(0..<3, id: \.self) { col in
                                if rowStart + col < ClockFaceTheme.allCases.count {
                                    themeCard(theme: ClockFaceTheme.allCases[rowStart + col])
                                } else {
